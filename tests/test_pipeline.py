    ...
    def test_train_baseline_api_endpoint(self):
        ...
        try:
            ...
            with urllib.request.urlopen(req) as resp:
                ...
                self.assertIn(data["status"], ["PASSED", "NEEDS_TUNING"])
        finally:
            server.shutdown()
            server.server_close()

    def test_train_advanced_ml_api_endpoint(self):
        import threading
        import time
        from api import HealthcareMLRequestHandler
        from http.server import HTTPServer

        server = HTTPServer(("127.0.0.1", 8099), HealthcareMLRequestHandler)
        thread = threading.Thread(target=server.serve_forever)
        thread.daemon = True
        thread.start()
        time.sleep(0.1)
        ...
        try:
            req = urllib.request.Request(
                "http://127.0.0.1:8099/train-advanced-ml",
                data=json.dumps({}).encode("utf-8"),
                headers={"Content-Type": "application/json"}
            )
            with urllib.request.urlopen(req) as resp:
                ...
                self.assertIn("comparative_metrics_matrix", data)
                self.assertIn(data["status"], ["PASSED", "NEEDS_TUNING"])
        finally:
            server.shutdown()
            server.server_close()

    def test_validate_leakage_free_ml_api_endpoint(self):
        import threading
        import time
        from api import HealthcareMLRequestHandler
        from http.server import HTTPServer

        server = HTTPServer(("127.0.0.1", 8100), HealthcareMLRequestHandler)
        thread = threading.Thread(target=server.serve_forever)
        thread.daemon = True
        thread.start()
        time.sleep(0.1)
        ...
        try:
            req = urllib.request.Request(
                "http://127.0.0.1:8100/validate-leakage-free-ml",
                data=json.dumps({}).encode("utf-8"),
                headers={"Content-Type": "application/json"}
            )
            with urllib.request.urlopen(req) as resp:
                self.assertEqual(resp.status, 200)
                data = json.loads(resp.read().decode("utf-8"))
                self.assertTrue(data["leakage_free_verified"])
                self.assertIn(data["status"], ["PASSED", "NEEDS_TUNING"])
        finally:
            server.shutdown()
            server.server_close()

    def test_validate_dataset_quality_api_endpoint(self):
        import threading
        import time
        from api import HealthcareMLRequestHandler
        from http.server import HTTPServer

        server = HTTPServer(("127.0.0.1", 8101), HealthcareMLRequestHandler)
        thread = threading.Thread(target=server.serve_forever)
        thread.daemon = True
        thread.start()
        time.sleep(0.1)
        ...
        try:
            req = urllib.request.Request(
                "http://127.0.0.1:8101/validate-dataset-quality",
                data=json.dumps({}).encode("utf-8"),
                headers={"Content-Type": "application/json"}
            )
            with urllib.request.urlopen(req) as resp:
                self.assertEqual(resp.status, 200)
                data = json.loads(resp.read().decode("utf-8"))
                self.assertIn("dataset_alignment_metrics", data)
